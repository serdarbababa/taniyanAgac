#TODO
#1. Girdileri farkli alacak
#2. Her  bir girdi selini okuyacak
...
class Girdi():
    ...
    def sygnal_generate(self, veriKaynagi="", parameters=[]):
        # parameters:   1. type of symbols [fixed, variable]
        #               2. number of symbols
        #               3. symbolSize
        if veriKaynagi == 'symbols' and parameters[0]=="fixed":
            self.auxiliaryData=[]
            symbol_data = symbolData()
            self.auxiliaryData.append(symbol_data )
            ops, ops_symbols=  self.auxiliaryData[0].generateOperationsSymbols(parameters[1])
            self.auxiliaryData.append(ops)
            self.auxiliaryData.append(ops_symbols)
            self.veri = self.auxiliaryData[0].mergeList(ops_symbols)

        elif veriKaynagi == 'symbols' and parameters[0]=="fixed with noise":
            self.auxiliaryData=[]
            symbol_data = symbolData()
            self.auxiliaryData.append(symbol_data )
            ops, ops_symbols=  self.auxiliaryData[0].generateOperationsSymbols(parameters[1])
    ...
    def onisleme(self, yontem = "None", parameters = []):
        self.onislenmis=[]
        if yontem == "None":
            self.onislenmis=self.veri
        elif( yontem == "wavelet"):
            for i in range(int (len(self.veri)/parameters[0] )):
               self.onislenmis.append( self.auxiliaryData[0].getWaveletCoefs(self.veri[i*parameters[0] : (i+1)*parameters[0]]))
            self.onislenmis = self.auxiliaryData[0].mergeList(self.onislenmis)
...
class symbolData():
    ...
    def __init__(self, symbol_size=16):
        #self.symbols = self.genSample(symbol_size)
